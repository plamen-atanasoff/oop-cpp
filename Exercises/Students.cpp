...
int getStudentsCount(std::ifstream& file)
{
    ...
    int counter = 0;
    // proper reading of files with or without '\n' at the end
    while (file.peek() != EOF)
    {
    ...
    // ! the state of the file is fail if there is no '\n' 
    // so we add clear to fix the file and make operations on it
    file.clear();
...
int main()
{
    // example file is "students.txt" in Resources
